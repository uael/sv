...
size_t
semver_fwrite(const semver_t *self, FILE *stream)
   ...
   set "errno" to an error code. */
{
  ...
  needed_count = (size_t) semver_pwrite(self, buffer_ptr, buffer_len);
  if (0 == needed_count) {
    return 0;
    ...
    if (NULL == buffer_ptr) {
      ...
      size_t actual_count = semver_pwrite(self, buffer_ptr, buffer_len);
...
size_t
semver_comp_fwrite(const semver_comp_t *self, FILE *stream)
   ...
   set "errno" to an error code. */
{
  ...
  needed_count = (size_t) semver_comp_pwrite(self, buffer_ptr, buffer_len);
  if (0 == needed_count) {
    return 0;
  } else if (needed_count < buffer_len) {
    errno = 0;
    ...
    if (NULL == buffer_ptr) {
      ...
      size_t actual_count = semver_comp_pwrite(self, buffer_ptr, buffer_len);
      size_t written_count;
      errno = 0;
      written_count = fwrite(buffer_ptr, sizeof(char), actual_count, stdout);
      free(buffer_ptr);
