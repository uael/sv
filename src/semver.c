...
char sv_read(sv_t *self, const char *str, size_t len, size_t *offset) {
  if (*offset < len) {
    *self = (sv_t) {0};
    ...
    if (sv_num_read(&self->major, str, len, offset) || *offset >= len || str[*offset] != '.'
      || sv_num_read(&self->minor, str, len, (++*offset, offset)) || *offset >= len || str[*offset] != '.'
      || sv_num_read(&self->patch, str, len, (++*offset, offset))
...
int sv_snprint(const sv_t self, char *buffer, size_t len) {
  ...
  if (self.prerelease.len && self.build.len) {
    return snprintf(buffer, len, "%d.%d.%d-%.*s+%.*s",
      ...
      sv_id_snprint(self.prerelease, prerelease, 256), prerelease,
      sv_id_snprint(self.build, build, 256), build
    );
  }
